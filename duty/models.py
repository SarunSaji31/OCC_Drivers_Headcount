...
class BusDetails(models.Model):
    ...
    def __str__(self):
        return f"Bus {self.bus_no} (Capacity: {self.seat_capacity})"

class ShiftDuty(models.Model):
    driver = models.ForeignKey(User, on_delete=models.CASCADE)  # Link to the user (driver)
    bus = models.ForeignKey(BusDetails, on_delete=models.CASCADE)  # Link to the bus
    start_km = models.DecimalField(max_digits=10, decimal_places=2)
    duty_card_no = models.CharField(max_length=50)
    created_at = models.DateTimeField(auto_now_add=True)

    def __str__(self):
        return f"Shift Duty for Bus {self.bus.bus_no} (Card: {self.duty_card_no})"
...
class TripLog(models.Model):
    driver = models.ForeignKey(User, on_delete=models.CASCADE)  # Ensure this field exists
    route_name = models.CharField(max_length=255)
    ...
    scanned_ids = models.PositiveIntegerField(default=0)
    scanned_staff_ids = models.JSONField(default=list)  

    def __str__(self):
        return f"Trip on {self.route_name} ({self.shift_duty.bus.bus_no})"
